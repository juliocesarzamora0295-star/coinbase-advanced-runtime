...
from src.marketdata.service import MarketDataService, CandleClosed, SymbolBarBuilder
...
class TestSymbolBarBuilderCalendar:
    ...
    def test_builder_handles_gaps(self):
        ...
        assert len(result) == 0, f"Expected 0 events (incomplete bucket with gap), got {len(result)}"
