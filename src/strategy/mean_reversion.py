...
from src.quantitative.indicators import bollinger_bands, rsi
...
class MeanReversionStrategy(Strategy):
    ...
    def generate_signals(self, *, mid: Decimal, bar_timestamp: Any = None) -> List:
        ...
        BUY when: close < lower Bollinger AND RSI < oversold
        SELL when: close > upper Bollinger AND RSI > overbought
        """
        ...
        if current_close <= current_lower and current_rsi < self.rsi_oversold:
            if self._last_signal_side != "buy":
                ...
                logger.info(
                    "BUY: close=%.2f <= BB_lower=%.2f, RSI=%.1f",
                    current_close,
                    current_lower,
                    current_rsi,
                )
