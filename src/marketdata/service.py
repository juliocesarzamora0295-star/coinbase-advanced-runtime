...
class SymbolBarBuilder:
            ...
            if self._current_bucket_id is not None and bucket_id != self._current_bucket_id:
                ...
                if self._current_bucket_candles:
                    if len(self._current_bucket_candles) == self.required_5m:
                        ...
                        event = self._build_candle_closed(
                            self._current_bucket_id, self._current_bucket_candles
                        )
                        if event:
                            events.append(event)
    ...
    def _build_candle_closed(
        self, bucket_id: int, candles_by_ts: Dict[int, Dict]
    ) -> Optional[CandleClosed]:
